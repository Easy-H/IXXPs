using System;
using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.Analytics;

public class TimeSet : MonoBehaviour {

    [SerializeField] Transform _target;
    [SerializeField] SpriteRenderer _background;
    [SerializeField] Light _light;

    [SerializeField] float _maxLight;
    [SerializeField] float _minLight;

    private void Start() {
        
    }

    private void Update() {
        
        Vector3 v = _target.forward;

        float realTime = (float)(System.DateTime.Now.Hour * 15 + System.DateTime.Now.Minute * 0.25);

        float timeAngle = Mathf.Atan2(v.x, v.z) * Mathf.Rad2Deg + realTime + 180;

        Debug.Log(timeAngle);

        if (timeAngle > 180) timeAngle -= 360;
        else if (timeAngle < -180) timeAngle += 360;

        Color color = _background.color;

        float alpha = (Mathf.Abs(timeAngle) - 60f) / 60;

        _background.color = new Color(color.r, color.g, color.b, alpha);

        _light.intensity = Mathf.Clamp(1 - alpha, _minLight, _maxLight);

    }

}
